<div align="center">

<img src="https://avatars.githubusercontent.com/u/142608496?v=4" width="100" height="100" style="border-radius: 50%; border: 2px solid #3178C6;" />

# Fahmi Aliyi
**Software Engineer | Full-Stack Developer**
Addis Ababa, Ethiopia

[Website](https://fahm.vercel.app) • [LinkedIn](https://linkedin.com/in/fahmcode) • [Email](mailto:fahmcode@gmail.com)

<br />

### Engineering performant, type-safe solutions.
I specialize in building reliable MVPs and developer tools with a focus on Clean Architecture and minimalist user experience.

<br />

<p>
  <img src="https://img.shields.io/badge/Go-00ADD8?style=for-the-badge&logo=go&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white" />
</p>

---

</div>

## Selected Engineering Projects

<table border="0">
  <tr>
    <td width="50%" valign="top">
      <h3>Saito</h3>
      <p><b>Mobile Engineering</b><br />
      A production-grade mobile application leveraging Clean Architecture principles to ensure scalability and ease of testing across platforms.</p>
      <p><i>Flutter, Dart, Provider</i></p>
      <a href="https://better-hub.com/fahmcode/saito">Technical Overview →</a>
    </td>
    <td width="50%" valign="top">
      <h3>Noor</h3>
      <p><b>Systems & Tooling</b><br />
      A high-performance CLI toolkit designed in Go to automate environment configurations and optimize local development cycles.</p>
      <p><i>Go, Cobra, Unix Shell</i></p>
      <a href="https://better-hub.com/fahmcode/noor">Technical Overview →</a>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>Glint</h3>
      <p><b>Browser Optimization</b><br />
      A minimalist Chrome extension focused on low-latency interactions. Built with native Browser APIs to ensure a near-zero memory footprint.</p>
      <p><i>JavaScript, Chrome Web APIs</i></p>
      <a href="https://better-hub.com/fahmcode/glint">Technical Overview →</a>
    </td>
    <td width="50%" valign="top">
      <h3>Dash</h3>
      <p><b>Frontend Architecture</b><br />
      A modern dashboard design system. Priority on accessibility (WCAG), responsive grid layouts, and reusable component logic.</p>
      <p><i>React, Next.js, TailwindCSS</i></p>
      <a href="https://better-hub.com/fahmcode/dash">Technical Overview →</a>
    </td>
  </tr>
</table>

<br />

## Technical Impact & Activity

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/fahmcode/fahmcode/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/fahmcode/fahmcode/output/github-contribution-grid-snake.svg">
    <img alt="Github contribution grid snake animation" src="https://raw.githubusercontent.com/fahmcode/fahmcode/output/github-contribution-grid-snake.svg">
  </picture>

  <br />

  <img src="https://github-readme-stats.vercel.app/api?username=fahmcode&show_icons=true&theme=react&hide_border=true&title_color=3178C6&icon_color=3178C6&count_private=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=fahmcode&layout=compact&theme=react&hide_border=true&title_color=3178C6" width="45%" />
</div>

<br />

---

<div align="center">

### Availability
I am currently open to new opportunities and technical collaborations. If you are looking for an engineer who values code quality and system performance, let's connect.

<br />

**Find me on**
<br />
[**GitHub**](https://github.com/fahmcode) &nbsp; | &nbsp; [**LinkedIn**](https://linkedin.com/in/fahmcode) &nbsp; | &nbsp; [**X / Twitter**](https://x.com/fahmcode) &nbsp; | &nbsp; [**Portfolio**](https://fahm.vercel.app)

<br />

<img src="https://komarev.com/ghpvc/?username=fahmcode&color=3178C6&label=PAGE+VIEWS&style=flat-square" />

<br />

<sub>&copy; 2026 Fahmi Aliyi • Built for performance</sub>

</div>
